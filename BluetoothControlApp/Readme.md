...
.venv/bin/python InstallDesktop.py
~~~

The build produces `dist/HydroponicsControl`. `InstallDesktop.py` installs the executable, icon, and launcher in the current user's application menu, and removes shortcuts created by earlier versions from `~/Desktop`. Re-run the installer after rebuilding to update the installed copy, then close and reopen the app. If you pinned the previous launcher, unpin it and pin the newly installed Hydroponics Control entry. The executable still needs Ubuntu's Bluetooth/BlueZ service and a display session.
